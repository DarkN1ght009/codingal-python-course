...
API = f"https://huggingface.co{MODEL}"
...
EMOJI = {
    ...
    "apple": "🍎", "orange": "🍊", "pizza": "🍕", "broccoli": "🥦", "book": "📖", "laptop": "💻",
    "tv": "📺", "bottle": "🍼", "cup": "🥤"
}
...
def infer(path, img_bytes, tries=3):
    mime, _ = mimetypes.guess_type(path)
    # Prefer exact image/* content-type; fall back to multipart if guess fails
    for _ in range(tries):
        if mime and mime.startswith("image/"):

            # (Inside a loop for API retries)
        else:
            r = requests.post(API,
                headers={"Authorization": f"Bearer {HF_API_KEY}"},
                files={"inputs": (os.path.basename(path), img_bytes,
...
def draw(img, dets, thr=0.5):
    ...
    for det in dets[:50]:
        ...
        # (Logic continues for drawing boxes based on 'b' coordinates)
