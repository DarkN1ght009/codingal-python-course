...
def smart_score(base, q1, q2):
    w1 = {w for w in clean(q1) if len(w) >= 4}
    w2 = {w for w in clean(q2) if len(w) >= 4}
    
    # Logic for 'strong' words (intersection of words >= 4 chars)
    strong = w1 & w2
    jac = len(strong) / max(1, len(w1 | w2))
...
def label(s): 
    return "✅ DUPLICATE" if s >= TH else ("🤨 CLOSE MATCH" if s >= TH - 0.05 else "❌ DIFFERENT")
...
def show_flow(q1, q2):
    ...
    conn = {
        "a", "an", "the", "to", "of", "in", "on", "is", "am", "are", "do", "did", "does",
        "my", "me", "it"
    }
    ...
    print(f" Q1 -> {TOK(q1)}"); print(f" Q2 -> {TOK(q2)}")
...
while True:
    ...
    try:
        ...
        for i, (d1, d2) in enumerate(random.sample(DEMOS, ...)): # Note: part of the line is cut off
